...
employee = { "Ann" : "ann", "Minna" : "minna", "Hira" : "hira", "Devika" : "devika", "Gopika" : "gopika"}


#connection to database
conn = pyodbc.connect('Driver={SQL Server Native Client 11.0};'
                      'Server=DESKTOP-QEJ3CSI;'
...
@cross_origin()
def Employees():
    data1 = pd.read_sql("SELECT * FROM Pharmacist", conn)
...
@cross_origin()
def Students():
    data2 = pd.read_sql("SELECT * FROM Hospital", conn)
...
@cross_origin()
def Branches():
    data3 = pd.read_sql("SELECT * FROM Doctor", conn)
...
@cross_origin()
def Details():
    data4 = pd.read_sql("SELECT * FROM Patients", conn)
